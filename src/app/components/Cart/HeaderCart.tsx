...
export default function HeaderCart() {
  ...
  return (
    <>
      {totalItens > 0 ? (
        <button
          className="flex relative items-center ml-10 pr-5 pl-3 pt-4 pb-3 border border-neutral-800 rounded-xl bg-neutral-300 transition cursor-pointer hover:scale-105 hover:shadow-xl"
          onClick={openModal}>
          <div className="absolute bg-neutral-400b w-5 h-5 rounded-full text-center end-1 top-1 text-sm font-bold">
            {totalItens}
          </div>
          <IoCartOutline size={30} className="my-auto" />
        ...
        <button
          className="flex relative items-center ml-10 p-4"
          onClick={openModal}>
          <IoCartOutline size={30} className="my-auto" />
          <strong className="my-auto ml-2">
            {formatarValor('pt-BR', 'BRL', valorTotal)}
